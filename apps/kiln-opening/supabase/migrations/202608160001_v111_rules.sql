-- Kiln Opening V1.1.1 authoritative rules/content transition.
-- Unstarted lobbies are safe to upgrade. Started older games remain immutable and are
-- rejected by the current Edge Function rather than translated mid-game: V1.1.1 changed
-- Glaze Preferred Heats, the Base Heat formula and range, and the kiln layout, so a
-- V1.0.9 board state has no faithful V1.1.1 reading.
...
alter table public.rooms add constraint rooms_rules_version_check
  check (rules_version in ('0.4', '0.5', '0.6.1', '0.6.3', '0.6.5', '1.0.0', '1.0.1', '1.0.2', '1.0.4', '1.0.9', '1.1.1'));
alter table public.rooms add constraint rooms_content_version_check
  check (content_version in ('0.4', '0.5', '0.6.1', '0.6.3', '0.6.5', '1.0.0', '1.0.1', '1.0.2', '1.0.4', '1.0.9', '1.1.1'));

update public.rooms
set rules_version = '1.1.1', content_version = '1.1.1', updated_at = now()
where status = 'lobby';

-- Payload shape is unchanged from 202608150002; only the stamped version moves.
-- The success key stays `seat`: AuthoritativeGameService.createRoom reads
-- created.value.seat, and the columns stay rp.is_ai with null AI details, because the
-- host seat can never be an AI seat.
...
begin
  ...
  values (p_room_id, upper(p_code), 'lobby', p_seat_id, '1.1.1', '1.1.1', 0);
