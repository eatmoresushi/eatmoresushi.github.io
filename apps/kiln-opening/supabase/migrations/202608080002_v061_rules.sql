-- Kiln Opening V0.6.1 room/content version transition.
-- Existing started V0.4/V0.5 games retain their original versions and are
-- rejected by the V0.6.1 service. Unstarted lobbies contain no game state and
-- are safe to move to the current version.
...
alter table public.rooms
  ...
  check (rules_version in ('0.4', '0.5', '0.6.1'));
alter table public.rooms
  add constraint rooms_content_version_check
  check (content_version in ('0.4', '0.5', '0.6.1'));

update public.rooms
set rules_version = '0.6.1', content_version = '0.6.1', updated_at = now()
where status = 'lobby';
...
begin
  insert into public.rooms (
    ...
    p_room_id, upper(p_code), 'lobby', p_seat_id, '0.6.1', '0.6.1', 0
  );
