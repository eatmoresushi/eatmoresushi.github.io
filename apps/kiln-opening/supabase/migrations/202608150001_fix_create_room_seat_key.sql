-- Restore the `seat` key in server_create_room's success payload.
--
-- 202608140001_v109_rules.sql renamed this key to `hostSeat`, but the store contract
-- has always been `{ room, seat }`: `AuthoritativeGameService.createRoom` reads
-- `created.value.seat` and passes it to `publicSeat`. With `hostSeat` the read is
-- undefined, `publicSeat` dereferences it, the Edge Function's catch converts the
-- TypeError into INTERNAL_SERVER_ERROR, and every attempt to create a room fails.
--
-- Nothing typechecked this: SQL is opaque to tsc, and the whole suite exercises the
-- in-memory store, which returns the correct shape. Only the deployed RPC differed.
-- server_join_room was never renamed, which is why joining kept working.
--
-- This is a payload-shape fix only. Rules/content versions, the room-code conflict
-- path, and every insert stay exactly as V1.0.9 defined them.
