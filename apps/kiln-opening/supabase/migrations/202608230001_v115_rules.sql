-- Kiln Opening v1.1.5 authoritative rules/content transition.
--
-- Unstarted lobbies upgrade; started v1.1.4 games stay immutable and are rejected by the
-- Edge Function rather than reinterpreted. v1.1.5 adds two uncapped action locations
-- (Labour and Court Patronage), removes the Office's Coin actions, rotates two Orders from
-- each display instead of one, drops Fuel Ledger's surcharge to 1 additional Wood, and
-- retunes four Kiln Traditions. A serialized v1.1.4 action board has no faithful v1.1.5
-- reading, so it is never translated.
...
alter table public.rooms add constraint rooms_rules_version_check
  check (rules_version in ('0.4', '0.5', '0.6.1', '0.6.3', '0.6.5', '1.0.0', '1.0.1', '1.0.2', '1.0.4', '1.0.9', '1.1.1', '1.1.4', '1.1.5'));
alter table public.rooms add constraint rooms_content_version_check
  check (content_version in ('0.4', '0.5', '0.6.1', '0.6.3', '0.6.5', '1.0.0', '1.0.1', '1.0.2', '1.0.4', '1.0.9', '1.1.1', '1.1.4', '1.1.5'));

update public.rooms
set rules_version = '1.1.5', content_version = '1.1.5', updated_at = now()
where status = 'lobby';

-- Only the stamped version moves; the payload shape is unchanged from 202608220001.
...
begin
  ...
  values (p_room_id, upper(p_code), 'lobby', p_seat_id, '1.1.5', '1.1.5', 0);
...
-- Computer seats keep the v1.1.4 Contribution policy: the Contribution card set is
-- unchanged in v1.1.5, so the policy is still correct for this ruleset.
