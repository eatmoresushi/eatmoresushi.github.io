-- Kiln Opening V0.6.5 room/content version transition.
-- Existing started games retain their recorded version and are rejected with a
-- clear incompatibility message. Lobby rooms contain no authoritative game
-- state, so they can be upgraded safely.
...
alter table public.rooms
  ...
  check (rules_version in ('0.4', '0.5', '0.6.1', '0.6.3', '0.6.5'));
alter table public.rooms
  add constraint rooms_content_version_check
  check (content_version in ('0.4', '0.5', '0.6.1', '0.6.3', '0.6.5'));

update public.rooms
set rules_version = '0.6.5', content_version = '0.6.5', updated_at = now()
where status = 'lobby';
...
begin
  insert into public.rooms (
    ...
    p_room_id, upper(p_code), 'lobby', p_seat_id, '0.6.5', '0.6.5', 0
  );
