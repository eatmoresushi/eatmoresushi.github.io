-- Point server_create_room's seat projection at columns that exist.
--
-- 202608140001_v109_rules.sql selected `rp.is_computer` and `rp.ai_policy_version`
-- from public.room_players. Neither column exists. 202608100002_online_ai_v003.sql
-- added exactly one AI marker to that table, `is_ai`, and kept the policy version,
-- seed and creating command in private.room_ai_seats. 202608150001 corrected the
-- payload key but copied these two references forward unchanged.
--
-- PL/pgSQL resolves column names when the body runs, not when the function is
-- created, so both migrations applied cleanly and then raised
-- `column rp.is_computer does not exist` on the first create_room. The RPC wrapper
-- turns that into a throw and the Edge Function reports INTERNAL_SERVER_ERROR.
--
-- The host seat is never an AI seat -- room_players_ai_not_host_check enforces
-- `not is_ai or not is_host` -- so the three AI detail fields are null by
-- construction here, exactly as server_get_seats reports them for a human seat.
