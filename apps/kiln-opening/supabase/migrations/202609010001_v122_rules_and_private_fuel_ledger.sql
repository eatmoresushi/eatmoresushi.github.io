-- Kiln Opening V1.2.2 save boundary and sealed Fuel Ledger commitments.
--
-- Existing rooms are deliberately not promoted. Their old version remains visible to the
-- service compatibility gate, which rejects them instead of silently starting or resuming
-- under different rules. Contribution cards and the Fuel Ledger flag remain in the private
-- schema until the simultaneous reveal.
...
alter table public.rooms add constraint rooms_rules_version_check
  check (rules_version in (
    ...
    '1.0.4', '1.0.9', '1.1.1', '1.1.4', '1.1.5', '1.1.6', '1.2.2'
  ));
...
-- V1.2.2 uses a rules-aware computer policy because setup, worker actions, Orders,
...
alter table private.room_ai_seats add constraint room_ai_seats_policy_version_check
  check (policy_version in (
    ...
    'rules-v1.1.5-order-001', 'rules-v1.2.2-heuristic-001'
  ));
...
begin
  ...
  if v_room.rules_version <> '1.2.2' or v_room.content_version <> '1.2.2' then
...
alter table private.private_submissions
  ...
  check (
    ...
    -- loaded by the V1.2.2 service, but migrations must not destroy valid audit history.
...
-- Keep the established fingerprint-aware signature and stamp every new room V1.2.2.
...
begin
  ...
  values (p_room_id, upper(p_code), 'lobby', p_seat_id, '1.2.2', '1.2.2', 0, p_content_digest);
...
-- Refuse any attempt to persist a pre-V1.2.2 or schema-1 initial state, even if a stale
...
begin
  ...
  if coalesce(v_room.rules_version, '') <> '1.2.2'
     or coalesce(v_room.content_version, '') <> '1.2.2'
     or coalesce(p_state->>'rulesVersion', '') <> '1.2.2'
     or coalesce(p_public_state->>'rulesVersion', '') <> '1.2.2'
...
begin
  ...
  if coalesce(p_next_state->>'rulesVersion', '') <> '1.2.2'
     or coalesce(p_public_state->>'rulesVersion', '') <> '1.2.2'
...
comment on column private.private_submissions.use_fuel_ledger is
  'Secret V1.2.2 extra-Wood commitment. Never expose through public state, events, PostgREST or Realtime.';
