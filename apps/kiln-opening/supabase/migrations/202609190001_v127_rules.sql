-- V1.2.7: new rooms only; preserve older rows, reject older rules and fingerprints.
...
alter table public.rooms add constraint rooms_rules_version_check
  check (rules_version in (
    ...
    '1.0.4', '1.0.9', '1.1.1', '1.1.4', '1.1.5', '1.1.6', '1.2.2', '1.2.4', '1.2.5', '1.2.6', '1.2.7'
  ));
...
alter table public.rooms add constraint rooms_v127_fingerprint_check
  check (
    ...
    or (
      ...
      and coalesce(content_digest, '') ~ '^r17-[0-9a-f]{16}$'
    )
  );
...
alter table private.room_ai_seats add constraint room_ai_seats_policy_version_check
  check (policy_version in (
    ...
    'rules-v1.2.6-heuristic-001', 'rules-v1.2.6-strategic-002', 'rules-v1.2.7-strategic-002'
  ));
...
begin
  ...
  if v_room.rules_version <> '1.2.7'
     or v_room.content_version <> '1.2.7'
     or coalesce(v_room.content_digest, '') !~ '^r17-[0-9a-f]{16}$' then
...
alter table private.private_submissions
  ...
  check (
    ...
    -- loaded by the V1.2.7 service, but migrations must not destroy valid audit history.
...
begin
  if coalesce(p_content_digest, '') !~ '^r17-[0-9a-f]{16}$' then
    return jsonb_build_object('status', 'error', 'code', 'session_not_active');
  end if;
  insert into public.rooms (id, code, status, host_seat_id, rules_version, content_version, latest_revision, content_digest)
  values (p_room_id, upper(p_code), 'lobby', p_seat_id, '1.2.7', '1.2.7', 0, p_content_digest);
...
begin
  ...
  if coalesce(v_room.rules_version, '') <> '1.2.7'
     or coalesce(v_room.content_version, '') <> '1.2.7'
     or coalesce(v_room.content_digest, '') !~ '^r17-[0-9a-f]{16}$'
     or coalesce(p_state->>'rulesVersion', '') <> '1.2.7'
     or coalesce(p_public_state->>'rulesVersion', '') <> '1.2.7'
...
begin
  ...
  if not exists (
       ...
       where room.id = p_room_id
         and room.rules_version = '1.2.7'
         and room.content_version = '1.2.7'
         and coalesce(room.content_digest, '') ~ '^r17-[0-9a-f]{16}$'
     )
     or coalesce(v_head.state_json->>'rulesVersion', '') <> '1.2.7'
     or coalesce((v_head.state_json->>'schemaVersion')::integer, -1) <> 4
     or coalesce(p_next_state->>'rulesVersion', '') <> '1.2.7'
     or coalesce(p_public_state->>'rulesVersion', '') <> '1.2.7'
