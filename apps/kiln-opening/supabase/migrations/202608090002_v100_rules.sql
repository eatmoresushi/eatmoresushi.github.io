-- Kiln Opening V1.0.0 room/content version transition.
-- Started games retain their recorded version and are rejected clearly. Lobby
-- rooms contain no authoritative game state, so they can be upgraded safely.
...
alter table public.rooms
  ...
  check (rules_version in ('0.4', '0.5', '0.6.1', '0.6.3', '0.6.5', '1.0.0'));
alter table public.rooms
  add constraint rooms_content_version_check
  check (content_version in ('0.4', '0.5', '0.6.1', '0.6.3', '0.6.5', '1.0.0'));

update public.rooms
set rules_version = '1.0.0', content_version = '1.0.0', updated_at = now()
where status = 'lobby';
...
begin
  insert into public.rooms (
    ...
    p_room_id, upper(p_code), 'lobby', p_seat_id, '1.0.0', '1.0.0', 0
  );
