-- Kiln Opening V1.2.5 save boundary.
--
-- V1.2.5 changes the serialized engine shape and several rules-bearing timing windows.
-- In particular all worker locations are shared, the recognition track ends at 4, and the
-- Kiln Yard Shifu repositions after Base Heat but before the Fire reveal. A V1.2.4 room
-- resumed under these rules would be reinterpreted mid-game, so existing rooms keep their
-- recorded version and the service/database compatibility gates refuse them rather than
-- promoting them silently.
--
-- Historical migrations are left untouched; this one replaces the current RPC functions.
...
alter table public.rooms add constraint rooms_rules_version_check
  check (rules_version in (
    ...
    '1.0.4', '1.0.9', '1.1.1', '1.1.4', '1.1.5', '1.1.6', '1.2.2', '1.2.4', '1.2.5'
  ));
...
alter table public.rooms drop constraint if exists rooms_v125_fingerprint_check;
alter table public.rooms add constraint rooms_v125_fingerprint_check
  check (
    rules_version <> '1.2.5'
    or (
      content_version = '1.2.5'
      and coalesce(content_digest, '') ~ '^r13-[0-9a-f]{16}$'
    )
  );

-- V1.2.5 uses a rules-aware computer policy because setup, worker actions, Orders,
...
begin
  ...
  if v_room.rules_version <> '1.2.5'
     or v_room.content_version <> '1.2.5'
     or coalesce(v_room.content_digest, '') !~ '^r13-[0-9a-f]{16}$' then
...
alter table private.private_submissions
  ...
  check (
    ...
    -- loaded by the V1.2.5 service, but migrations must not destroy valid audit history.
...
-- Keep the established fingerprint-aware signature and stamp every new room V1.2.5.
...
begin
  if coalesce(p_content_digest, '') !~ '^r13-[0-9a-f]{16}$' then
    return jsonb_build_object('status', 'error', 'code', 'session_not_active');
  end if;
  insert into public.rooms (id, code, status, host_seat_id, rules_version, content_version, latest_revision, content_digest)
  values (p_room_id, upper(p_code), 'lobby', p_seat_id, '1.2.5', '1.2.5', 0, p_content_digest);
...
-- Refuse any attempt to persist a pre-V1.2.5 or pre-schema-3 initial state, even if a stale
...
begin
  ...
  if coalesce(v_room.rules_version, '') <> '1.2.5'
     or coalesce(v_room.content_version, '') <> '1.2.5'
     or coalesce(v_room.content_digest, '') !~ '^r13-[0-9a-f]{16}$'
     or coalesce(p_state->>'rulesVersion', '') <> '1.2.5'
     or coalesce(p_public_state->>'rulesVersion', '') <> '1.2.5'
     or coalesce((p_state->>'schemaVersion')::integer, -1) <> 3
     or coalesce((p_public_state->>'schemaVersion')::integer, -1) <> 3 then
    return jsonb_build_object('status', 'error', 'code', 'session_not_active');
  end if;
...
begin
  ...
  if not exists (
       ...
       where room.id = p_room_id
         and room.rules_version = '1.2.5'
         and room.content_version = '1.2.5'
         and coalesce(room.content_digest, '') ~ '^r13-[0-9a-f]{16}$'
     )
     or coalesce(v_head.state_json->>'rulesVersion', '') <> '1.2.5'
     or coalesce((v_head.state_json->>'schemaVersion')::integer, -1) <> 3
     or coalesce(p_next_state->>'rulesVersion', '') <> '1.2.5'
     or coalesce(p_public_state->>'rulesVersion', '') <> '1.2.5'
     or coalesce((p_next_state->>'schemaVersion')::integer, -1) <> 3
     or coalesce((p_public_state->>'schemaVersion')::integer, -1) <> 3 then
...
comment on column private.private_submissions.use_fuel_ledger is
  'Secret Fuel Ledger extra-Wood commitment. Never expose through public state, events, PostgREST or Realtime.';
