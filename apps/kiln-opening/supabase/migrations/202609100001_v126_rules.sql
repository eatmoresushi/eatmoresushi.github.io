-- Kiln Opening V1.2.6 save boundary.
--
-- V1.2.6 changes the serialized engine shape and two rules-bearing interactions.
-- In particular the Kiln Yard Shifu commits a specific Shared-Kiln ceramic during the
-- worker action, and Ding's bonus Shape costs 1 Clay independently. A V1.2.5 room
...
alter table public.rooms add constraint rooms_rules_version_check
  check (rules_version in (
    ...
    '1.0.4', '1.0.9', '1.1.1', '1.1.4', '1.1.5', '1.1.6', '1.2.2', '1.2.4', '1.2.5', '1.2.6'
  ));
...
alter table public.rooms add constraint rooms_v126_fingerprint_check
  check (
    ...
    or (
      ...
      and coalesce(content_digest, '') ~ '^r14-[0-9a-f]{16}$'
    )
  );

-- V1.2.6 uses a rules-aware computer policy because setup, worker actions, Orders,
...
alter table private.room_ai_seats add constraint room_ai_seats_policy_version_check
  check (policy_version in (
    ...
    'rules-v1.1.5-order-001', 'rules-v1.2.2-heuristic-001', 'rules-v1.2.4-heuristic-001', 'rules-v1.2.5-heuristic-001',
    'rules-v1.2.6-heuristic-001'
  ));
...
alter table private.private_submissions
  ...
  check (
    ...
    -- loaded by the V1.2.6 service, but migrations must not destroy valid audit history.
...
-- Keep the established fingerprint-aware signature and stamp every new room V1.2.6.
...
begin
  if coalesce(p_content_digest, '') !~ '^r14-[0-9a-f]{16}$' then
    return jsonb_build_object('status', 'error', 'code', 'session_not_active');
  end if;
  insert into public.rooms (id, code, status, host_seat_id, rules_version, content_version, latest_revision, content_digest)
  values (p_room_id, upper(p_code), 'lobby', p_seat_id, '1.2.6', '1.2.6', 0, p_content_digest);
...
-- Refuse any attempt to persist a pre-V1.2.6 or pre-schema-4 initial state, even if a stale
...
begin
  ...
  if coalesce(v_room.rules_version, '') <> '1.2.6'
     or coalesce(v_room.content_version, '') <> '1.2.6'
     or coalesce(v_room.content_digest, '') !~ '^r14-[0-9a-f]{16}$'
     or coalesce(p_state->>'rulesVersion', '') <> '1.2.6'
     or coalesce(p_public_state->>'rulesVersion', '') <> '1.2.6'
     or coalesce((p_state->>'schemaVersion')::integer, -1) <> 4
     or coalesce((p_public_state->>'schemaVersion')::integer, -1) <> 4 then
    return jsonb_build_object('status', 'error', 'code', 'session_not_active');
  end if;
...
begin
  ...
  if not exists (
       ...
       where room.id = p_room_id
         ...
         and room.content_version = '1.2.6'
         and coalesce(room.content_digest, '') ~ '^r14-[0-9a-f]{16}$'
     )
     or coalesce(v_head.state_json->>'rulesVersion', '') <> '1.2.6'
     or coalesce((v_head.state_json->>'schemaVersion')::integer, -1) <> 4
     or coalesce(p_next_state->>'rulesVersion', '') <> '1.2.6'
     or coalesce(p_public_state->>'rulesVersion', '') <> '1.2.6'
     or coalesce((p_next_state->>'schemaVersion')::integer, -1) <> 4
     or coalesce((p_public_state->>'schemaVersion')::integer, -1) <> 4 then
