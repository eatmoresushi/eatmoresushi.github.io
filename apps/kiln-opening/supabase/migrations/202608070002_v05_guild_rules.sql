-- Kiln Opening V0.5 room/content version transition.
-- Existing V0.4 games remain tagged V0.4 and are rejected by the V0.5 service;
-- unstarted lobbies are safe to upgrade because they contain no game state.
...
alter table public.rooms
  add constraint rooms_rules_version_check check (rules_version in ('0.4', '0.5'));
alter table public.rooms
  add constraint rooms_content_version_check check (content_version in ('0.4', '0.5'));

update public.rooms
set rules_version = '0.5', content_version = '0.5', updated_at = now()
where status = 'lobby';
...
begin
  insert into public.rooms (
    ...
    p_room_id, upper(p_code), 'lobby', p_seat_id, '0.5', '0.5', 0
  );
