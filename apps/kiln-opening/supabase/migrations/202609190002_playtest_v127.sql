-- Accept new V1.2.7 Form V2 playtests without rewriting historical V1.2.4 or V1.2.5 rows.
...
-- Resource supplies are unlimited. The numeric bound only preserves exact JSON
-- integers across JavaScript clients; it is not a physical component limit.
-- This view depends on the resource column types and is recreated below.
drop view if exists private.playtest_player_summary;

alter table private.playtest_players
  drop constraint if exists playtest_players_coins_remaining_check,
  drop constraint if exists playtest_players_clay_remaining_check,
  drop constraint if exists playtest_players_wood_remaining_check,
  alter column coins_remaining type bigint,
  alter column clay_remaining type bigint,
  alter column wood_remaining type bigint;

alter table private.playtest_players
  add constraint playtest_players_coins_remaining_check
    check (coins_remaining between 0 and 9007199254740991),
  add constraint playtest_players_clay_remaining_check
    check (clay_remaining between 0 and 9007199254740991),
  add constraint playtest_players_wood_remaining_check
    check (wood_remaining between 0 and 9007199254740991);
...
alter table private.playtest_submissions
  ...
  check (rules_version in ('1.2.4', '1.2.5', '1.2.6', '1.2.7'));

alter table private.playtest_submissions
  drop constraint if exists playtest_submissions_v127_form_check;

alter table private.playtest_submissions
  add constraint playtest_submissions_v127_form_check
  check (rules_version <> '1.2.7' or form_version = 2);
...
begin
  ...
  if jsonb_typeof(p_payload) <> 'object'
    or coalesce(p_payload->>'rulesVersion', '') <> '1.2.7'
  ...
  insert into private.playtest_submissions (
    ...
    v_submission_id, v_game_number, v_game_id, v_form_version, '1.2.7',
  ...
  loop
    ...
    insert into private.playtest_players (
      ...
      (v_player->>'coinsRemaining')::bigint,
      (v_player->>'clayRemaining')::bigint,
      (v_player->>'woodRemaining')::bigint
    );
...
select
  ...
  player.coin_vp,
  case
    when submission.rules_version in ('1.2.5', '1.2.6', '1.2.7') and player.recognition = 4 then 6::smallint
  ...
  case
    when submission.rules_version in ('1.2.5', '1.2.6', '1.2.7') then
...
-- Include the rules version in every analysis/export view so V1.2.4, V1.2.5, and V1.2.7
