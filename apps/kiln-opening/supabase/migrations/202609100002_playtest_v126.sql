-- Accept new V1.2.6 Form V2 playtests without rewriting historical V1.2.4 or V1.2.5 rows.
...
alter table private.playtest_submissions
  ...
  check (rules_version in ('1.2.4', '1.2.5', '1.2.6'));

alter table private.playtest_submissions
  drop constraint if exists playtest_submissions_v126_form_check;

alter table private.playtest_submissions
  add constraint playtest_submissions_v126_form_check
  check (rules_version <> '1.2.6' or form_version = 2);
...
begin
  ...
  if jsonb_typeof(p_payload) <> 'object'
    or coalesce(p_payload->>'rulesVersion', '') <> '1.2.6'
  ...
  insert into private.playtest_submissions (
    ...
    v_submission_id, v_game_number, v_game_id, v_form_version, '1.2.6',
...
select
  ...
  player.coin_vp,
  case
    when submission.rules_version in ('1.2.5', '1.2.6') and player.recognition = 4 then 6::smallint
  ...
  case
    when submission.rules_version in ('1.2.5', '1.2.6') then
...
-- Include the rules version in every analysis/export view so V1.2.4, V1.2.5, and V1.2.6
