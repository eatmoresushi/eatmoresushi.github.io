-- Kiln Opening v1.1.4 authoritative rules/content transition.
--
-- Unstarted lobbies are safe to upgrade. Started v1.1.1 games remain immutable and are
-- rejected by the Edge Function rather than translated mid-game: v1.1.4 replaced the
-- numeric 0-3 Wood Contribution with three named Contribution cards, removed the
-- contributor-count formula and the Kiln Yard Wood grant, changed the 2-player kiln to
-- 2 High / 1 Middle / 2 Low, moved Fuel Ledger to a post-reveal window, and removed the
-- Imperial Progress Coin stipends. A serialized v1.1.1 board has no faithful v1.1.4
-- reading, so it is never reinterpreted.
...
alter table public.rooms add constraint rooms_rules_version_check
  check (rules_version in ('0.4', '0.5', '0.6.1', '0.6.3', '0.6.5', '1.0.0', '1.0.1', '1.0.2', '1.0.4', '1.0.9', '1.1.1', '1.1.4'));
alter table public.rooms add constraint rooms_content_version_check
  check (content_version in ('0.4', '0.5', '0.6.1', '0.6.3', '0.6.5', '1.0.0', '1.0.1', '1.0.2', '1.0.4', '1.0.9', '1.1.1', '1.1.4'));

update public.rooms
set rules_version = '1.1.4', content_version = '1.1.4', updated_at = now()
where status = 'lobby';

-- A contribution is now a card, not a number. `contribution` stays for already-recorded
-- v1.1.1 rows, so it becomes nullable rather than being dropped; every v1.1.4 row writes
-- `contribution_card` instead. Exactly one of the two is present on any row.
alter table private.private_submissions add column if not exists contribution_card text;
alter table private.private_submissions alter column contribution drop not null;
alter table private.private_submissions drop constraint if exists private_submissions_contribution_card_check;
alter table private.private_submissions add constraint private_submissions_contribution_card_check
  check (contribution_card is null or contribution_card in ('BANK', 'TEND', 'STOKE'));
alter table private.private_submissions drop constraint if exists private_submissions_contribution_present_check;
alter table private.private_submissions add constraint private_submissions_contribution_present_check
  check (num_nonnulls(contribution, contribution_card) = 1);

-- Readers return the card. The sealed submission no longer carries a Fuel Ledger flag: v1.1.4
-- resolves Fuel Ledger after the reveal, so it is never committed with the card.
...
-- New rooms are stamped v1.1.4. Payload shape is otherwise identical to 202608160001:
-- the success key stays `seat` because AuthoritativeGameService.createRoom reads
-- created.value.seat, and the columns stay rp.is_ai with null AI details.
...
begin
  ...
  values (p_room_id, upper(p_code), 'lobby', p_seat_id, '1.1.4', '1.1.4', 0);
...
-- Commit path: persist the Contribution card instead of a 0-3 amount.
...
begin
  ...
  if p_private_submission is not null then
    ...
    insert into private.private_submissions (
      room_id, window_id, player_id, command_id, contribution_card
    ) values (
      p_room_id, v_window_id, p_actor_id, p_command_id,
      p_private_submission->>'card');
  end if;
