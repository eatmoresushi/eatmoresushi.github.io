...
The **current V1.0.1 gameplay source of truth is `docs/GAME_RULES.md`**. Visual references remain restricted to `assets/current_v04/`; the legacy directory name is retained only for stable paths. Every rules-bearing visual must be regenerated or covered by live current-rule UI from V1.0.1 data. The V0.4 PDF/DOCX in `source_rulebook/` are historical human-readable backups and are superseded by `GAME_RULES.md`.
...
Earlier Order-card, central-board, player-board, Craft Technique and reference-card raster art is intentionally **not authoritative** where any text or component count differs from current V1.0.1 rules. See `docs/ASSET_MANIFEST.md`.
...
The V1.0.1 implementation includes the strict TypeScript engine, server-authoritative Supabase backend, React/Vite interface, reconnect handling, the symmetric five-value Fire deck, blind Order draws, player-scaled kiln spaces, the expanded Orders/Techniques, gated Court Patronage, and deterministic two-browser Playwright game tests.
...
See `docs/V0.4_HANDOFF_AUDIT.md` for the historical component-by-component V0.4 audit. Run `python3 tools/validate_handoff.py` to validate the current V1.0.1 structured rules.
