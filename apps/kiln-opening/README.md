...
The **current gameplay source of truth is `docs/GAME_RULES.md`**. This audited edition also restricts visual assets to `assets/current_v04/`; every other visual must be regenerated from V0.4 data. The PDF/DOCX in `source_rulebook/` are human-readable backup copies of the same V0.4 rules.
...
Earlier central-board, player-board, Craft Technique and reference-card raster art is intentionally **not included** where any text or component count differs from V0.4. See `docs/ASSET_MANIFEST.md`.
...
The V0.4 implementation now includes the strict TypeScript engine, server-authoritative Supabase backend, React/Vite interface, reconnect handling, and a deterministic two-browser Playwright game test.
...
See `docs/V0.4_HANDOFF_AUDIT.md` for the component-by-component V0.4 audit. Run `python tools/validate_handoff.py` before implementation.
