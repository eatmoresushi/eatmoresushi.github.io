...
export function createdGame(playerCount: PlayerCount, seed = 122): StartedGame {
  ...
  const result = createGame(
    { gameId: `v127-${playerCount}-${seed}`, players: playerInputs(playerCount) },
    rng,
  );
  if (!result.ok) throw new Error(result.error.message);
...
/** Advance a firing fixture by legally spending every remaining worker at Labour. */
export function finishWork(state: GameState, rng: SeededRandom): Extract<ApplyResult, { ok: true }> {
  const events: Extract<ApplyResult, { ok: true }>["events"] = [];
  let next = state;
  while (next.phase.type === "work" || next.phase.type === "work_imperial_priority") {
    const actor = currentDecisionActor(next.phase)!;
    const action: GameAction = next.phase.type === "work_imperial_priority"
      ? { type: "RESOLVE_IMPERIAL_PRIORITY", ceramicId: null }
      : { type: "USE_LABOUR", workerId: Object.values(next.players[actor]!.workers).find((worker) => worker.status === "available")!.id };
    const result = mustResult(next, actor, action, rng);
    next = result.state;
