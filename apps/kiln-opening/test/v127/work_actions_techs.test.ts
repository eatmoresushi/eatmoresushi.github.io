...
describe("V1.2.7 worker actions and Techs", () => {
  ...
  it("pays full Materials Yard and Labour gains above physical component counts", () => {
    const { state, rng } = startedGame(2, 1301_1);
    for (const player of Object.values(state.players)) player.resources = { clay: 100, wood: 100, coins: 100 };
    const materialsResult = mustResult(state, "P1", {
      type: "GAIN_MATERIALS", workerId: workerId(state, "P1", "shifu"), clay: 2, wood: 2, buyShifuBonus: true,
    }, rng);
    const materials = materialsResult.state;
    expect(materials.players["P1"]!.resources).toEqual({ clay: 103, wood: 103, coins: 99 });
    expect(materialsResult.events).toContainEqual({ type: "RESOURCES_CHANGED", playerId: "P1", clay: 3, wood: 3, coins: -1 });
    setWorkTurn(materials, "P1");
    const labour = mustApply(materials, "P1", {
      type: "USE_LABOUR", workerId: workerId(materials, "P1", "apprentice"),
    }, rng);
    expect(labour.players["P1"]!.resources.coins).toBe(101);
  });
  ...
  it("rejects passing with unused workers without changing state", () => {
    const { state, rng } = startedGame(2, 1316);
  ...
  it("implements all five Forming Advanced Tech effects", () => {
    ...
    expect(state.players["P1"]!.resources.clay).toBe(beforeClay);
  ...
  it("lets Colour Samples reserve a looked-at Order or a face-up one, discarding the rest", () => {
    ...
    expect(display.marketDisplay).toHaveLength(6);
  });
