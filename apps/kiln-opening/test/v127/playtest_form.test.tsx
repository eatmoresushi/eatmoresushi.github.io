...
import v127PlaytestMigration from "../../supabase/migrations/202609190002_playtest_v127.sql?raw";
...
describe("V1.2.7 playtest form", () => {
  ...
  it.each([0, 100_000, Number.MAX_SAFE_INTEGER])("accepts resource reports of %s without a physical supply cap", (amount) => {
    ...
    for (const key of ["coinsRemaining", "clayRemaining", "woodRemaining"]) {
      players[0]![key] = amount;
    }
    ...
    expect(result.value.players[0]).toMatchObject({
      ...
      clayRemaining: amount,
      woodRemaining: amount,
    });
  });

  it.each([-1, 1.5, Infinity, NaN, Number.MAX_SAFE_INTEGER + 1, "100", null])("rejects invalid resource reports of %s", (amount) => {
    const candidate = validCandidate() as Record<string, unknown>;
    const players = candidate["players"] as Array<Record<string, unknown>>;
    const keys = ["coinsRemaining", "clayRemaining", "woodRemaining"];
    for (const key of keys) players[0]![key] = amount;
    const result = validatePlaytestSubmission(candidate);
    expect(result.ok).toBe(false);
    if (result.ok) return;
    for (const key of keys) {
      expect(result.issues.some((entry) => entry.path === `players.0.${key}`)).toBe(true);
    }
  });

  it("renders resource inputs without a maximum while keeping the Coin VP cap", () => {
    const markup = renderToStaticMarkup(createElement(PlaytestFormPage));
    for (const resource of ["Coins", "Clay", "Wood"]) {
      const inputs = [...markup.matchAll(new RegExp(`<span>${resource}</span>(<input[^>]+>)`, "g"))];
      expect(inputs).toHaveLength(2);
      for (const input of inputs) {
        expect(input[1]).toContain('min="0"');
        expect(input[1]).not.toContain("max=");
      }
    }
    expect(markup).toMatch(/<span>Coin VP<\/span><input[^>]+max="5"/);
  });
  ...
  it("shows the derived Imperial Recognition score", () => {
    ...
    expect(v127PlaytestMigration).toContain("when submission.rules_version in ('1.2.5', '1.2.6', '1.2.7') and player.recognition = 4 then 6");
  });
  ...
  it("stores concise submissions privately and exposes only a service-role RPC", () => {
    ...
    expect(v127PlaytestMigration).toContain("coalesce(p_payload->>'rulesVersion', '') <> '1.2.7'");
    expect(v127PlaytestMigration).toContain("v_form_version <> 2");
    expect(v127PlaytestMigration).toContain("advanced_tech_vp");
    for (const resource of ["coins", "clay", "wood"]) {
      expect(v127PlaytestMigration).toContain(`drop constraint if exists playtest_players_${resource}_remaining_check`);
      expect(v127PlaytestMigration).toContain(`alter column ${resource}_remaining type bigint`);
      expect(v127PlaytestMigration).toContain(`check (${resource}_remaining between 0 and 9007199254740991)`);
      expect(v127PlaytestMigration).toContain(`(v_player->>'${resource}Remaining')::bigint`);
    }
