...
describe("V1.2.7 firing, Tech timing, and Kiln Traditions", () => {
  ...
  it("does not treat passing as a worker action for Imperial Priority timing", () => {
    ...
    state.players["P1"]!.imperialPriorityAvailable = true;
    expectError(applyAction(state, "P1", { type: "PASS_WORK_PHASE" }, rng), "INVALID_ACTION");
    expect(state.phase).toEqual({ type: "work", activePlayerId: "P1" });
  ...
  it("resolves Jun at 1 Wood and rejects the retired Ge heat adjustment", () => {
  ...
  it("lets an unused Jun ability resolve during a Second Firing recalculation", () => {
