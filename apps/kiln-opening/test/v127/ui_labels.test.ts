...
/**
 * What the player reads must be what V1.2.7 calls it.
 *
 * V1.2.7 renamed three locations but kept their ids, and the ids are the only thing the
 ...
 * `data/action_locations.json`, and no UI string may name a mechanic V1.2.7 dropped.
 */
const UI_DIR = join(import.meta.dirname, "../../src/ui");

/** Terms the V1.2.7 source uses zero times. Value is the name that replaced it. */
const RETIRED_VOCABULARY: Array<[RegExp, string]> = [
  ...
  [/Sagger Selection/, "removed; V1.2.7 has Protective Saggars"],
  ...
  [/Workshop Seconds/, "V1.2.7 calls it the 2-Coin discard of a still-Flawed ceramic"],
...
describe("V1.2.7 user-facing labels", () => {
  ...
  it("carries the three V1.2.7 renames through to the player", () => {
  ...
  it("names no mechanic V1.2.7 dropped, in any UI string", () => {
