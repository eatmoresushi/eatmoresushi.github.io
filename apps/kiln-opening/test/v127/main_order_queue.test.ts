...
const DISPLAY = ["O01", "O02", "O03", "O04", "O05", "O48"] satisfies OrderId[];
...
describe("V1.2.7 ordered Main Order display", () => {
  it.each([
    {
      label: "oldest leftmost card",
      orderId: "O01" as const,
      expected: ["O02", "O03", "O04", "O05", "O48", "O06"],
    },
    {
      label: "middle card",
      orderId: "O03" as const,
      expected: ["O01", "O02", "O04", "O05", "O48", "O06"],
    },
    {
      label: "newest rightmost card",
      orderId: "O48" as const,
      expected: ["O01", "O02", "O03", "O04", "O05", "O06"],
    },
  ...
  it("resolves both Shifu reservations against the newly updated queue", () => {
    ...
    expect(state.marketDisplay).toEqual(["O01", "O02", "O04", "O05", "O48", "O06"]);
    ...
    expect(state.marketDisplay).toEqual(["O01", "O02", "O04", "O05", "O48", "O07"]);
  ...
  it("slides and appends when Colour Samples reserves a face-up Order", () => {
    ...
    expect(state.marketDisplay).toEqual(["O01", "O02", "O04", "O05", "O48", "O09"]);
  ...
  it("uses unchosen Colour Samples cards to refill a face-up reservation after the deck is exhausted", () => {
    ...
    expect(state.marketDisplay.slice(0, 5)).toEqual(["O01", "O02", "O04", "O05", "O48"]);
    expect(state.marketDisplay).toHaveLength(6);
    expect(lookedAt).toContain(state.marketDisplay[5]);
    ...
    expect(relevantCards).toHaveLength(9);
    expect(new Set(relevantCards).size).toBe(9);
  });
  ...
  it.each([2, 3, 4, 5] as const)(
    ...
    (round) => {
      ...
      expect(state.marketDisplay).toEqual(["O03", "O04", "O05", "O48", "O06", "O07"]);
