...
describe("V1.2.7 online computer policy: Commission Market", () => {
  it("reserves the top Main Order unseen when nothing face up is deliverable", async () => {
    ...
    state.marketDeck = state.marketDeck.filter((id) => !state.marketDisplay.includes(id));
...
describe("V1.2.7 online computer policy: which Tech it buys", () => {
  ...
  it("buys a valuable inspected tile over a worthless face-up one", async () => {
    ...
    // Both inspected tiles cost 2; the policy values Second Firing more highly.
    expect(await choose(inspected, "P1")).toEqual(expect.objectContaining({ techniqueId: "T14" }));
  });
});
