 ...
 * why the drift survived: the outputs that would have exposed it stayed correct. That
 * directory is gone now, but the pattern it demonstrated is what these tests exist to catch.
 *
...
describe("authoritative content has exactly one source", () => {
  ...
  it("resolves every Order and Technique from the data files", () => {
    ...
    expect(Object.keys(ORDER_DEFINITIONS).length).toBe(MAIN_ORDERS.length + STARTING_ORDERS.length);
