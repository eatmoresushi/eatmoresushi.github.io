...
import migration from "../../supabase/migrations/202609030001_v124_rules.sql?raw";
import supabaseStore from "../../supabase/functions/_shared/supabaseStore.ts?raw";

describe("V1.2.4 Supabase contract", () => {
  it("stamps new rooms and rejects old save schemas at both commit boundaries", () => {
    expect(migration).toContain("'1.2.4', '1.2.4', 0, p_content_digest");
    ...
    expect(migration).toContain("coalesce(p_next_state->>'rulesVersion', '') <> '1.2.4'");
    expect(migration).not.toMatch(/update public\.rooms[\s\S]{0,240}set rules_version = '1\.2\.4'/);
  });
  ...
  it("installs the current computer policy and keeps the function service-role-only", () => {
    expect(migration).toContain("'rules-v1.2.4-heuristic-001'");
    expect(migration).toContain("create or replace function public.server_add_computer_seat");
    expect(migration).toContain("v_room.rules_version <> '1.2.4'");
