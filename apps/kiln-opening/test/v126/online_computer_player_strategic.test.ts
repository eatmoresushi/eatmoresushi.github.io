...
describe("V1.2.6 strategic computer policy: Starting Techs and Ding", () => {
...
describe("V1.2.6 strategic computer policy: route-repair Techs", () => {
  ...
  it("uses Glaze Palette to repair another unloaded ceramic while glazing", async () => {
    ...
    wrongGlaze.loadableFromRound = state.round + 1;
    ...
    expect(action).toEqual(expect.objectContaining({
      type: "GLAZE_CERAMICS",
      useTechniqueIds: expect.arrayContaining(["T06"]),
      glazePalette: expect.objectContaining({ ceramicId: wrongGlaze.id }),
    }));
    if (action.type !== "GLAZE_CERAMICS" || action.glazePalette === undefined) return;
    expect(["white", "moon_white"]).toContain(action.glazePalette.glaze);

    const resolved = mustApply(state, "P1", action, rng);
    expect(resolved.ceramics[wrongGlaze.id]).toEqual(expect.objectContaining({
      stage: "glazed",
      glaze: action.glazePalette.glaze,
    }));
    expect(resolved.players["P1"]!.techniques).toContainEqual({ id: "T06", exhausted: true });
  });
});

describe("V1.2.6 strategic computer policy: firing placement", () => {
...
describe("V1.2.6 strategic computer policy: audited timing and limits", () => {
  it("discards an owned Flawed firing result when the 2-Coin salvage is available", async () => {
    ...
    state.commonSupply.coins = Math.max(2, state.commonSupply.coins);
    state.firingContext = {
      round: state.round,
      contributors: ["P1"],
      contributions: { P1: "TEND" },
