...
describe("V1.2.6 eligibility for before-Quality Kiln ability prompts", () => {
  it.each([
  ...
  it.each([
    { offset: -1, baseHeat: 1 },
    { offset: 1, baseHeat: 3 },
  ] as const)("offers Ge for exactly $offset Heat from Preferred Heat, even at 0 Wood", ({ baseHeat }) => {
    const { state, rng } = revealFixture(baseHeat);
    state.players["P1"]!.kilnId = "GE";
    const ceramic = addLoaded(state, "P1", "bowl", "celadon", "plain", "middle_1");
    const revealed = mustResult(state, "P1", { type: "REVEAL_FIRE_CARD" }, rng);

    expect(revealed.state.phase).toEqual({ type: "firing_before_quality", queue: { actors: ["P1"], currentIndex: 0 } });
    expect(revealed.events.some((event) => event.type === "QUALITY_ASSIGNED")).toBe(false);
    expect(renderPanel(revealed.state)).toContain(kilnShortPlainText("GE", "en"));
    const corrected = mustResult(revealed.state, "P1", { type: "RESOLVE_GE", ceramicId: ceramic.id }, rng);
    expect(corrected.state.ceramics[ceramic.id]).toEqual(expect.objectContaining({
      stage: "finished", quality: "masterpiece", decoration: "crackle",
    }));
    expect(corrected.state.players["P1"]!.resources.wood).toBe(0);
    expect(corrected.events).toContainEqual({ type: "KILN_ABILITY_USED", playerId: "P1", kilnId: "GE" });
  });
  ...
  it("keeps eligible Ge and Jun decisions clockwise from the First Player", () => {
    ...
    let next = mustApply(state, "P2", { type: "REVEAL_FIRE_CARD" }, rng);
    expect(next.phase).toEqual({ type: "firing_before_quality", queue: { actors: ["P3", "P1"], currentIndex: 0 } });
    next = mustApply(next, "P3", { type: "RESOLVE_JUN", ceramicId: null, delta: null }, rng);
    expect(next.phase).toEqual({ type: "firing_before_quality", queue: { actors: ["P3", "P1"], currentIndex: 1 } });
    const result = mustResult(next, "P1", { type: "RESOLVE_GE", ceramicId: null }, rng);
    expect(result.events.filter((event) => event.type === "QUALITY_ASSIGNED")).toHaveLength(2);
  });
