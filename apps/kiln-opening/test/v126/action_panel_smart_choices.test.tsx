...
function expectUnavailable(markup: string, explanation: string | RegExp): void {
  ...
  expect(markup).toContain("Pass for this round");
}
...
describe("V1.2.6 smart worker-action choices", () => {
  ...
  it("keeps Labour gain copy concise and its submit button consistent", () => {
    ...
    const partialSupply = renderAction("labour", "shifu", (state) => {
      state.commonSupply.coins = 1;
    }).markup;

    expect(apprentice).toContain("Gain 2 Coins.");
    expect(shifu).toContain("Gain 4 Coins.");
    expect(partialSupply).toContain("Gain 1 Coin.");
  ...
  it("replaces Labour controls with an explanation when the common Coin supply is empty", () => {
    const { markup } = renderAction("labour", "apprentice", (draft) => {
      draft.commonSupply.coins = 0;
    });

    expectUnavailable(markup, /Coins.*supply/i);
  });
  ...
  it("replaces Materials controls only when both common resource supplies are empty", () => {
    const { markup } = renderAction("materials_yard", "apprentice", (draft) => {
      draft.commonSupply.clay = 0;
      draft.commonSupply.wood = 0;
    });

    expectUnavailable(markup, /Clay.*Wood.*supply/i);
  });

  it.each(["clay", "wood"] as const)("keeps Materials usable with just one %s remaining", (resource) => {
    const { markup, selectedWorkerId } = renderAction("materials_yard", "apprentice", (draft) => {
      draft.commonSupply.clay = 0;
      draft.commonSupply.wood = 0;
      draft.commonSupply[resource] = 1;
    });
  ...
  it("keeps Labour usable when less than a worker's normal reward remains", () => {
    const { markup, selectedWorkerId } = renderAction("labour", "shifu", (draft) => {
      draft.commonSupply.coins = 1;
    });
