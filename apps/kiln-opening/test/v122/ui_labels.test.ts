...
/**
 * What the player reads must be what V1.2.2 calls it.
 *
 * V1.2.2 renamed three locations but kept their ids, and the ids are the only thing the
 ...
 * `data/action_locations.json`, and no UI string may name a mechanic V1.2.2 dropped.
 */
const UI_DIR = join(import.meta.dirname, "../../src/ui");

/** Terms the V1.2.2 source uses zero times. Value is the name that replaced it. */
const RETIRED_VOCABULARY: Array<[RegExp, string]> = [
  ...
  [/Sagger Selection/, "removed; V1.2.2 has Protective Saggars"],
...
describe("V1.2.2 user-facing labels", () => {
  ...
  it("carries the three V1.2.2 renames through to the player", () => {
    expect(LOCATION_DEFINITIONS.market_imperial_office.name).toBe("Commission Market");
    expect(LOCATION_DEFINITIONS.forming_studio.name).toBe("Private Potter's Wheel");
    expect(LOCATION_DEFINITIONS.glaze_workshop.name).toBe("Private Glaze & Decoration");
  });

  it("names no mechanic V1.2.2 dropped, in any UI string", () => {
