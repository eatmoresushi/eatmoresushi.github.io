...
Current **v1.1.4** compatibility telemetry records the round each Progress space 1-5 is first reached, Progress source (Order or Patronage), Exhibition capacity/use/Quality/diversity scoring, Imperial Seal acquisition, one-card Order-display rotations, Fire-modifier and Quality distributions, Jun/Ge windows, the Contribution card each contributor revealed with the Wood it cost, post-reveal Fuel Ledger upgrades, Test Pieces and Sagger Selection Wood activations, Shifu Material exchanges, Clay Substitution, Drying Frames, merged Shifu free Decorations, and Connoisseur sales at 3/6/10. v1.1.4 removed the Progress 2 and 4 Coin stipends and the per-ceramic Kiln Yard Wood grant, so those columns no longer populate. Historical V1.0.1, V1.0.2, V1.0.4 and V1.0.9 exports remain immutable.
